...
class ExcelWriter(object):
    ...
    #----------------------------------------------------------------------
    def writeToCell(self, row, col, cell_content, sheet_name='Content'):
        self.table = self.excel_file.add_sheet(sheet_name, cell_overwrite_ok=True)
        table.write(row, col, cell_content)
...
class BacktestingEngine(object):
    ...
    def loadHistoryData(self):
        ...
        self.output(u'载入完成，数据量：%s' %(initCursor.count() + 
                                             self.dbCursor.count()))
    ...
    def runBacktesting(self):
        ...
        self.data_list = []
        for i, d in enumerate(self.dbCursor):
            data = dataClass()
            data.__dict__ = d
            data.index = i
    ...
    def calculateBacktestingResult(self):
        ...
        fff = open('results_list.dat', 'w')
        fff1 = open('pnl_sum.dat', 'w')
        for result in resultList:
            fff.write(str(result))
            capital += result.pnl
            fff.write(' current capital is ' + str(capital) + '\n')
            fff1.write(str(capital) + '\n')
    ...
    #----------------------------------------------------------------------
    def writeToExcel(self, col):
        pass
        
    #----------------------------------------------------------------------
    def showBacktestingResult(self):
        """显示回测结果"""
        self.d = self.calculateBacktestingResult()
        
        # 输出
        self.output('-' * 30)
        self.output(u'第一笔交易：\t%s' % self.d['timeList'][0])
        self.output(u'最后一笔交易：\t%s' % self.d['timeList'][-1])
        
        self.output(u'总交易次数：\t%s' % formatNumber(self.d['totalResult']))        
        self.output(u'总盈亏：\t%s' % formatNumber(self.d['capital']))
        self.output(u'最大回撤: \t%s' % formatNumber(min(self.d['drawdownList'])))                
        
        self.output(u'平均每笔盈利：\t%s' %formatNumber(
                        self.d['capital']/self.d['totalResult']))
        self.output(u'平均每笔滑点：\t%s' %formatNumber(
                        self.d['totalSlippage']/self.d['totalResult']))
        self.output(u'平均每笔佣金：\t%s' %formatNumber(
                        self.d['totalCommission']/self.d['totalResult']))
        
        self.output(u'胜率\t\t%s%%' %formatNumber(self.d['winningRate']))
        self.output(u'平均每笔盈利\t%s' %formatNumber(self.d['averageWinning']))
        self.output(u'平均每笔亏损\t%s' %formatNumber(self.d['averageLosing']))
        self.output(u'盈亏比：\t%s' %formatNumber(self.d['profitLossRatio']))
        ...
        pCapital.plot(self.d['capitalList'])
        
        pDD = plt.subplot(3, 1, 2)
        pDD.set_ylabel("DD")
        pDD.bar(range(len(self.d['drawdownList'])), self.d['drawdownList'])         
        
        pPnl = plt.subplot(3, 1, 3)
        pPnl.set_ylabel("pnl")
        pPnl.hist(self.d['pnlList'], bins=50)
    ...
    def runParallelOptimization(self, strategyClass, optimizationSetting):
        ...
        for setting in settingList:
            ...
            l.append(pool.apply_async(optimize, (strategyClass, setting,
                                                 ...
                                                 self.startDate, self.initDays,
                                                 self.endDate, self.slippage, 
                                                 self.rate, self.size,
...
class TradingResult(object):
    ...
    def __str__(self):
        repr_str = '\t'.join(['entryPrice:', str(self.entryPrice),
                              'exitPrice:', str(self.exitPrice),
                              'entryDt: ', str(self.entryDt),
                              'exitDt:', str(self.exitDt),
...
if __name__ == '__main__':
    ...
    # 设置回测用的数据起始日期
    engine.setStartDate('20100417')
    # 设置回测用的数据结束日期
    engine.setEndDate('20170328')
    
    
    engine.setDatabase('VnTrader_5Min_Db', 'IF00')
    
    engine.setSlippage(0)     # 股指1跳
    engine.setRate(1/10000)   # 万0.3 # orginal
    engine.setSize(300)         # 股指合约大小
    
    # 在引擎中创建策略对象
    engine.initStrategy(DualThrustTSG, {})
